        ...
        if (par1GuiButton.enabled)//enabled
        {
            switch (par1GuiButton.id)//id
            {
            	...
            	case 16: /* Shadow Resolution Multiplier */
            	{
            		...
            		float[] choices = { 0.25f,0.3333333333f,0.5f,0.7071067812f,1.0f,1.414213562f,2.0f,3.0f,4.0f };
            		int i;
            		if (!isShiftKeyDown()) {
            			for (i=0; i<choices.length && choices[i]<=val; ++i) {}
                		if (i==choices.length) i=0;
            	...
            	case 5: /* Open shaderpacks folder */
                    ...
                    try
                    {
                        ...
                        var3.getMethod("browse", new Class[] {URI.class}).invoke(var4, new Object[] {(new File(mc.mcDataDir, Shaders.shaderpacksdirname)).toURI()});
                    }
