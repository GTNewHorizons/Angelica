...
/**
 * The state a vanilla glint block leaves behind, for callers that skip the block. GLSM is only called where its
 * effective state differs.
 */
public final class SkippedGlintBlock {
    ...
    private static void applyExitState(float red, float green, float blue, float alpha, int srcRgb, int dstRgb, int srcAlpha, int dstAlpha,
                                       boolean resetTextureMatrix, boolean restoreDepthMask) {
        ...
        if (GLStateManager.getBlendState().isFuncUnknown() || blend.getSrcRgb() != srcRgb || blend.getDstRgb() != dstRgb
            || blend.getSrcAlpha() != srcAlpha || blend.getDstAlpha() != dstAlpha) {
