...
public class GLStateManagerFogService implements FogService {
    ...
    public float getFogEnd() {
        return GL33C.glGetInteger(GL11.GL_FOG_END);
    }

    @Override
    public float getFogStart() {
        return GL33C.glGetInteger(GL11.GL_FOG_START);
    }

    @Override
    public float getFogDensity() {
        return GL33C.glGetFloat(GL11.GL_FOG_DENSITY);
    }
    ...
    public ChunkFogMode getFogMode() {
        if (!GL33C.glGetBoolean(GL11.GL_FOG)) {
            return ChunkFogMode.NONE;
        }
        return ChunkFogMode.fromGLMode(GL33C.glGetInteger(GL11.GL_FOG_MODE));
    }
}
