...
public interface ObjectByteConsumer<T> extends BiConsumer<T, Byte>
{
	 ...
	 */
	public default ObjectByteConsumer<T> andThen(ObjectByteConsumer<T> after) {
	...
	@Override
	@Deprecated
	default void accept(T k, Byte v) { accept(k, v.byteValue()); }
	...
	@Override
	@Deprecated
	default ObjectByteConsumer<T> andThen(BiConsumer<? super T, ? super Byte> after) {
		Objects.requireNonNull(after);
		return (K, V) -> {accept(K, V); after.accept(K, Byte.valueOf(V));};
	}
}