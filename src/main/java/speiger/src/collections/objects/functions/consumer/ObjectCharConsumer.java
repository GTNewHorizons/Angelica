...
public interface ObjectCharConsumer<T> extends BiConsumer<T, Character>
{
	 ...
	 */
	public default ObjectCharConsumer<T> andThen(ObjectCharConsumer<T> after) {
	...
	@Override
	@Deprecated
	default void accept(T k, Character v) { accept(k, v.charValue()); }
	...
	@Override
	@Deprecated
	default ObjectCharConsumer<T> andThen(BiConsumer<? super T, ? super Character> after) {
		Objects.requireNonNull(after);
		return (K, V) -> {accept(K, V); after.accept(K, Character.valueOf(V));};
	}
}