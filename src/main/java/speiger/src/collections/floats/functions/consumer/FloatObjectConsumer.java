...
public interface FloatObjectConsumer<V> extends BiConsumer<Float, V>
{
	 ...
	 */
	public default FloatObjectConsumer<V> andThen(FloatObjectConsumer<V> after) {
	...
	@Override
	@Deprecated
	default void accept(Float k, V v) { accept(k.floatValue(), v); }
	...
	@Override
	@Deprecated
	default FloatObjectConsumer<V> andThen(BiConsumer<? super Float, ? super V> after) {
		Objects.requireNonNull(after);
		return (K, V) -> {accept(K, V); after.accept(Float.valueOf(K), V);};
	}
}