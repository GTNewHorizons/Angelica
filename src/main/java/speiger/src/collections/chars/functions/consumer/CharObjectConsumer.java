...
public interface CharObjectConsumer<V> extends BiConsumer<Character, V>
{
	 ...
	 */
	public default CharObjectConsumer<V> andThen(CharObjectConsumer<V> after) {
	...
	@Override
	@Deprecated
	default void accept(Character k, V v) { accept(k.charValue(), v); }
	...
	@Override
	@Deprecated
	default CharObjectConsumer<V> andThen(BiConsumer<? super Character, ? super V> after) {
		Objects.requireNonNull(after);
		return (K, V) -> {accept(K, V); after.accept(Character.valueOf(K), V);};
	}
}