...
@AllArgsConstructor
@EqualsAndHashCode
public class BlockMetaEntry implements Entry {

    private final NamespacedId id;
    private final IntSet metas;

    private static final Pattern META_LIST = Pattern.compile("^\\d+(,\\d+)*$");
    ...
    public static Entry parse(@NotNull String entry) {
        ...
        if (isMeta) {
            IntSet metas = new IntOpenHashSet();

            for (int index = statesStart; index < splitStates.length; index++) {
                for (String meta : splitStates[index].split(",")) {
                    try {
                        metas.add(Integer.parseInt(meta));
                    } catch (NumberFormatException e) {
                        Iris.logger.warn("Warning: the block ID map entry \"" + entry + "\" could not be fully parsed:");
                        Iris.logger.warn("- Meta filters must be an integer, but " + meta + " is not of that form!");
                    }
                }
            }

            return new BlockMetaEntry(id, metas);
        }
        ...
        return new BlockPropEntry(id, map);
    }
}
