...
/**
 * Owns Iris's color and depth render targets and the framebuffers that wire them together for
 * the deferred rendering pipeline. Supports both monoscopic ({@code eyeCount == 1}) and
 * stereoscopic ({@code eyeCount == 2}) operation; for stereo, each eye gets its own color textures
 * and its own depth-plus-stencil texture so kernel-based composite shaders (bloom, blur, SSAO)
 * don't bleed across the eye boundary. The {@link #setActiveEye(int)} call re-attaches every
 * owned framebuffer to the new eye's textures so a single {@link GlFramebuffer} object handed
 * out to a {@link net.coderbot.iris.postprocess.CompositeRenderer} pass simply targets the
 * current eye after each switch.
 *
 * <p>For stereo, {@link #blitEyeDepthStencilToMain} copies an eye's depth+stencil into that eye's
 * region of the main framebuffer's depth+stencil texture at the end of the eye's pipeline. This
 * preserves stencil-using mods (Avaritia cosmic textures, Forge stencil effects, Thaumcraft):
 * each eye's mod stencil writes go into a real per-eye stencil during world rendering, and the
 * blit-back makes them visible to any post-world-render code that reads main FB stencil.</p>
 */
public class RenderTargets {
	...
	/**
	 * Allocates an Iris-owned depth+stencil (or depth-only) texture matching the requested format
	 * — used for stereo per-eye depth so each eye has its own stencil buffer during world rendering.
	 */
	...
	/**
	 * Switches which eye's textures the owned framebuffers point at. No-op for mono. Caller must
	...
	/**
	 * Clears the active eye's depth+stencil texture. No-op in monoscopic mode (where Iris shares
	 * MC's main framebuffer depth and MC's per-frame glClear handles it). In stereo mode each
	 * eye owns a private depth+stencil texture that MC never touches, so we have to clear it
	 * ourselves between eye passes — otherwise stale depth/stencil from the previous frame
	 * causes wrong occlusion and stencil test failures.
	 */
	...
	/**
	 * Copies the current eye's depth+stencil texture into the eye's region of the main framebuffer's
	 * depth+stencil texture. Called by the pipeline at the end of each eye's deferred + composite +
	 * final pass when stereo is active. Preserves stencil so any code that reads main FB stencil
	 * after world rendering (vanilla, mods, the second eye's pre-pass) sees a coherent state.
	 *
	 * <p>Source region: full eye texture {@code (0,0)-(cachedWidth,cachedHeight)}. Destination region
	 * is the rectangle within the main FB's depth+stencil texture passed in by the caller.</p>
	 *
	 * @param mainFbGlId the GL framebuffer ID of the main MC framebuffer (already has the main
	 *                   depth+stencil texture attached)
	 * @param dstX0      destination region x0 (in main FB pixel coords)
	 * @param dstY0      destination region y0
	 * @param dstX1      destination region x1 (exclusive)
	 * @param dstY1      destination region y1 (exclusive)
	 */
