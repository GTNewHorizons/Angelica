...
public class ProfileElementWidget extends BaseOptionElementWidget<OptionMenuProfileElement> {
	private static final MutableComponent PROFILE_LABEL = I18n.format("options.iris.profile");
	private static final MutableComponent PROFILE_CUSTOM = I18n.format("options.iris.profile.custom").withStyle(ChatFormatting.YELLOW);
	...
	public void render(PoseStack poseStack, int x, int y, int width, int height, int mouseX, int mouseY, float tickDelta, boolean hovered) {
		this.updateRenderParams(width, width - (Minecraft.getMinecraft().font.width(PROFILE_LABEL) + 16));
