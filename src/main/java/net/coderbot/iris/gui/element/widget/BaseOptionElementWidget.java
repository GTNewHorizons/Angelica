...
public abstract class BaseOptionElementWidget<T extends OptionMenuElement> extends CommentedElementWidget<T> {
	protected static final Component SET_TO_DEFAULT = I18n.format("options.iris.setToDefault").withStyle(ChatFormatting.GREEN);
	...
	protected final void renderTooltip(PoseStack poseStack, Component text, int mouseX, int mouseY, boolean hovered) {
		if (hovered) {
			ShaderPackScreen.TOP_LAYER_RENDER_QUEUE.add(() -> GuiUtil.drawTextPanel(Minecraft.getMinecraft().font, poseStack, text, mouseX + 2, mouseY - 16));
		}
	}
	...
	public Optional<Component> getCommentBody() {
		return Optional.ofNullable(getCommentKey()).map(key -> I18n.exists(key) ? I18n.format(key) : null);
	}
