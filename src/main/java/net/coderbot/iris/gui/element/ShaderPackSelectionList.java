...
public class ShaderPackSelectionList extends IrisObjectSelectionList<ShaderPackSelectionList.BaseEntry> {
	private static final Component PACK_LIST_LABEL = I18n.format("pack.iris.list.label").withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY);
	...
	public static class LabelEntry extends BaseEntry {
		...
		@Override
		public void render(PoseStack poseStack, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
			drawCenteredString(poseStack, Minecraft.getMinecraft().font, label, (x + entryWidth / 2) - 2, y + (entryHeight - 11) / 2, 0xC2C2C2);
		}
	}

	public static class TopButtonRowEntry extends BaseEntry {
		private static final Component REFRESH_SHADER_PACKS_LABEL = I18n.format("options.iris.refreshShaderPacks").withStyle(style -> style.withColor(TextColor.fromRgb(0x99ceff)));
		private static final Component NONE_PRESENT_LABEL = I18n.format("options.iris.shaders.nonePresent").withStyle(ChatFormatting.GRAY);
		private static final Component SHADERS_DISABLED_LABEL = I18n.format("options.iris.shaders.disabled");
		private static final Component SHADERS_ENABLED_LABEL = I18n.format("options.iris.shaders.enabled");
		...
		public void render(PoseStack poseStack, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
			...
			if (this.refreshPacksButton.isHovered()) {
				ShaderPackScreen.TOP_LAYER_RENDER_QUEUE.add(() ->
						GuiUtil.drawTextPanel(Minecraft.getMinecraft().font, poseStack, REFRESH_SHADER_PACKS_LABEL,
								(mouseX - 8) - Minecraft.getMinecraft().font.width(REFRESH_SHADER_PACKS_LABEL), mouseY - 16));
			}
		}
