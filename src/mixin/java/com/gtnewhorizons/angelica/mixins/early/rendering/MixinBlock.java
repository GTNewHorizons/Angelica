...
public class MixinBlock implements ThreadedBlockData.Getter {
    ...
    public final ThreadedBlockData angelica$getThreadData() {
        final ThreadedBlockData data = angelica$threadData.get();
        if (data != null) return data;
        return angelica$createData();
    }

    @Unique
    private ThreadedBlockData angelica$createData() {
        final ThreadedBlockData data;
        synchronized (this) {
